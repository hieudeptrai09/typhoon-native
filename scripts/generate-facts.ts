/**
 * Builds the fun-fact list and stores it in the `facts` table. Run with `npm run facts`, and re-run
 * whenever the storm data changes.
 *
 * This is a script rather than a query because assembling the list takes 51 queries, and the server
...
async function generateFacts(): Promise<string[]> {
  ...
  rowsResult = await rows(
    `SELECT name, year FROM storms WHERE intensity = 'MD' ORDER BY year, name`,
  );
  if (rowsResult.length > 0) {
    const items = rowsResult.map((r) => `${r.name} (${r.year})`);
    facts.push(
  ...
  rowsResult = await rows(`
    ...
    WHERE s.intensity = 'MD' AND s.jtwcnumber IS NOT NULL
...
async function main(): Promise<void> {
  ...
  // An empty list almost certainly means a broken query or an empty database, and replacing the
  // stored facts with nothing would leave the app with none. Stop instead.
  if (facts.length === 0) {
    throw new Error("Generated no facts — refusing to empty the table.");
  }

  // Swap the whole set in one transaction so a reader never sees a half-filled table.
  await client.begin(async (tx) => {
    await tx.unsafe("DELETE FROM facts");
    await tx.unsafe("INSERT INTO facts (text) SELECT unnest($1::text[])", [facts]);
  });
