...
describe("isValidStormsSlug", () => {
  ...
  it("accepts a filter only when the view offers it", () => {
    ...
    expect(isValidStormsSlug(["intensity", "5"])).toBe(false); // URLs key on the slug, not the enum
...
describe("getLegendKind", () => {
  ...
  it("shows the intensity scale only where color tracks intensity", () => {
    ...
    expect(kind("intensity", "md", "table")).toBe("intensity");
    expect(kind("intensity", "cat5", "list")).toBe("intensity");
  });
